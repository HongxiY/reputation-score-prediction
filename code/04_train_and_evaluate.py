...
Input : processed/pooled_dataset.json
        data/submission_template.csv           (read-only; the 28 holdout dates)
Output: processed/model_results.json           metrics, coefficients, predictions
        ...
        processed/submission_template.csv      the template plus a filled
                                               reputation_score column

Usage:
    python code/04_train_and_evaluate.py
"""
...
def main() -> int:
    ...
    # the filled copy under processed/. The template itself is not written.
